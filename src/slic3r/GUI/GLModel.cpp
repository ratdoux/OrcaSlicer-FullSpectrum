...
void GLModel::Geometry::add_vertex(const Vec3f& position, const Vec3f& normal, const RGBA& color)
{
...
{
    return format.vertex_layout == EVertexLayout::P3N3C4;
}
...
void GLModel::render(const std::pair<size_t, size_t>& range)
{
    ...
    if (color) {
        ...
        if (color_id != -1) {
            glsafe(::glVertexAttribPointer(color_id, Geometry::color_stride_floats(data.format), GL_FLOAT, GL_FALSE,
                                           vertex_stride_bytes, (const void*)Geometry::color_offset_bytes(data.format)));
            glsafe(::glEnableVertexAttribArray(color_id));
        }
    }

    shader->set_uniform("uniform_color", data.color);
    shader->set_uniform("use_vertex_color", color);
