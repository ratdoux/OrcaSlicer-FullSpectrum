    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
...

    {
        ...
        p->m_sidebar_filament_menu->set_on_delete_mixed([this](int index) {
            p->plater->sidebar().delete_mixed_filament(size_t(index));
        });
        ...
        p->m_sidebar_filament_menu->set_on_action(SidebarFilamentMenu::ActionType::DeleteFilament, [this]() {
            p->plater->sidebar().delete_filament(size_t(-1));
        });
...
bool Sidebar::is_filament_in_use(unsigned int filament_1based) const
{
    if (filament_1based == 0)
        return false;

    // Check all objects and volumes
    for (const ModelObject* mo : wxGetApp().model().objects) {
        if (!mo) continue;

        if (!mo->config.has("extruder") || mo->config.extruder() == 0) {
            if (filament_1based == 1)
                return true;
        } else if (mo->config.extruder() == int(filament_1based)) {
            return true;
        }
        ...
        for (const char* key : keys) {
            if (mo->config.has(key)) {
                if (filament_1based == 1 && mo->config.opt_int(key) == 0)
                    return true;
                if (mo->config.opt_int(key) == int(filament_1based))
                    return true;
            }
        }
        ...
        for (const auto& layer_range : mo->layer_config_ranges) {
            if (layer_range.second.has("extruder")) {
                if (filament_1based == 1 && layer_range.second.option("extruder")->getInt() == 0)
                    return true;
                if (layer_range.second.option("extruder")->getInt() == int(filament_1based))
                    return true;
            }
        }
        ...
        for (const ModelVolume* mv : mo->volumes) {
            ...
            if (mv->config.has("extruder")) {
                if (filament_1based == 1 && mv->config.extruder() == 0)
                    return true;
                if (mv->config.extruder() == int(filament_1based))
                    return true;
            }
            for (const char* key : keys) {
                if (mv->config.has(key)) {
                    if (filament_1based == 1 && mv->config.opt_int(key) == 0)
                        return true;
                    if (mv->config.opt_int(key) == int(filament_1based))
                        return true;
                }
            }
            std::vector<int> used_extruders = mv->get_extruders();
            if (std::find(used_extruders.begin(), used_extruders.end(), int(filament_1based)) != used_extruders.end())
                return true;
        }
    }

    // Check custom gcodes
    for (const auto& item : wxGetApp().model().plates_custom_gcodes) {
        for (const auto& gcode_item : item.second.gcodes) {
            if (gcode_item.type == CustomGCode::Type::ToolChange && gcode_item.extruder == int(filament_1based))
                return true;
        }
    }
...
void Sidebar::transfer_filament(size_t from_id, size_t to_id)
{
    if (from_id == size_t(-2))
    ...
    const unsigned int from_1based = static_cast<unsigned int>(from_id + 1);
    const unsigned int to_1based   = static_cast<unsigned int>(to_id + 1);

    plater->take_snapshot(_u8L("Transfer Filament"));

    // 1. Remap painting facets in ModelVolumes
    EnforcerBlockerStateMap state_map;
    for (size_t i = 0; i < state_map.size(); ++i)
        state_map[i] = EnforcerBlockerType(i);
    state_map[from_1based] = EnforcerBlockerType(to_1based);
    ...
    // 2. Transfer object and volume extruder configs & feature filaments
    static const char* keys[] = {"wall_filament", "sparse_infill_filament", "solid_infill_filament",
                                 "support_filament", "support_interface_filament"};
    ...
    for (ModelObject* mo : wxGetApp().model().objects) {
        if (!mo->config.has("extruder") || mo->config.extruder() == 0) {
            if (from_1based == 1) {
                mo->config.set_key_value("extruder", new ConfigOptionInt(int(to_1based)));
            }
        } else if (mo->config.extruder() == int(from_1based)) {
            mo->config.set_key_value("extruder", new ConfigOptionInt(int(to_1based)));
        }

        for (const char* key : keys) {
            if (mo->config.has(key)) {
                if (from_1based == 1 && mo->config.opt_int(key) == 0) {
                    mo->config.set_key_value(key, new ConfigOptionInt(int(to_1based)));
                } else if (mo->config.opt_int(key) == int(from_1based)) {
                    mo->config.set_key_value(key, new ConfigOptionInt(int(to_1based)));
                }
            }
        }
        ...
        for (ModelVolume* mv : mo->volumes) {
            if (mv->config.has("extruder")) {
                if (from_1based == 1 && mv->config.extruder() == 0) {
                    mv->config.set_key_value("extruder", new ConfigOptionInt(int(to_1based)));
                } else if (mv->config.extruder() == int(from_1based)) {
                    mv->config.set_key_value("extruder", new ConfigOptionInt(int(to_1based)));
                }
            }
            for (const char* key : keys) {
                if (mv->config.has(key)) {
                    if (from_1based == 1 && mv->config.opt_int(key) == 0) {
                        mv->config.set_key_value(key, new ConfigOptionInt(int(to_1based)));
                    } else if (mv->config.opt_int(key) == int(from_1based)) {
                        mv->config.set_key_value(key, new ConfigOptionInt(int(to_1based)));
                    }
                }
            }
        ...
        for (auto& range : mo->layer_config_ranges) {
            if (range.second.has("extruder")) {
                if (from_1based == 1 && range.second.option("extruder")->getInt() == 0) {
                    range.second.set("extruder", int(to_1based));
                } else if (range.second.option("extruder")->getInt() == int(from_1based)) {
                    range.second.set("extruder", int(to_1based));
                }
            }
        }
    ...
    // 3. Update plate custom gcodes
    for (auto& item : wxGetApp().model().plates_custom_gcodes) {
        for (auto& gcode_item : item.second.gcodes) {
            if (gcode_item.type == CustomGCode::Type::ToolChange && gcode_item.extruder == int(from_1based)) {
                gcode_item.extruder = int(to_1based);
            }
        }
    }

    // 4. Update UI / ObjectList
    obj_list()->transfer_filament_values_for_items(from_id, to_id);
...
void Sidebar::merge_mixed_filament(size_t from_id, size_t to_id)
{
    ...
    // 1. Transfer all references from source mixed filament to target filament
    transfer_filament(from_id, to_id);

    // 2. Delete source mixed filament definition
    delete_mixed_filament(size_t(source_mixed_idx));
}

void Sidebar::delete_mixed_filament(size_t mixed_index)
{
    PresetBundle* preset_bundle = wxGetApp().preset_bundle;
    if (!preset_bundle) return;
    auto& pb = *preset_bundle;
    ...
    if (mixed_index >= definitions.size())
        return;

    // Find the 0-based virtual ID of this mixed filament
    size_t visible_card_idx = 0;
    bool found = false;
    ...
    size_t mixed_virtual_id = num_physical + visible_card_idx; // 0-based
    unsigned int mixed_virtual_1based = (unsigned int)(mixed_virtual_id + 1);

    bool in_use = is_filament_in_use(mixed_virtual_1based);

    if (in_use) {
        // Build choices of surviving filaments
        std::vector<FilamentChoiceItem> choices;
        std::vector<wxBitmap*> icons = get_extruder_color_icons(true);
        const auto& filament_presets = pb.filament_presets;
        int filaments_cnt = std::min((int)icons.size(), (int)filament_presets.size());
        for (int i = 0; i < filaments_cnt; i++) {
            auto preset = pb.filaments.find_preset(filament_presets[i]);
            wxString item_name = preset ? from_u8(preset->label(false)) : wxString::Format(_L("Filament %d"), i + 1);
            choices.push_back({ item_name, *icons[i], size_t(i) });
        }
        ...
        std::vector<std::string> physical_colors;
        if (auto* co2 = pb.project_config.option<ConfigOptionStrings>("filament_colour"))
        ...
        for (size_t j = 0; j < mfs.size(); ++j) {
            ...
            size_t target_vid = num_physical + running_idx;
            running_idx++;
            if (target_vid == mixed_virtual_id) continue;
            ...
            wxBitmap* mixed_bmp = create_mixed_filament_menu_bitmap(mfs[j], menu_ctx, icon_width, icon_height, wxString(letter));
            choices.push_back({ item_name, *mixed_bmp, target_vid });
        }

        wxString msg = _L("This mixed filament is currently in use.\nAre you sure you want to delete it?");
        DeleteFilamentDialog dlg(wxGetApp().mainframe, _L("Confirm Delete"), msg, true, choices, 0);
        if (dlg.ShowModal() != wxID_OK) {
            return;
        }

        int target_id = dlg.get_target_filament_id();
        if (target_id >= 0) {
            transfer_filament(mixed_virtual_id, size_t(target_id));
        }
    }

    // Delete definition
    auto& target = definitions[mixed_index];
    auto canonical_pair = [](unsigned int a, unsigned int b) {
        return std::make_pair(std::min(a, b), std::max(a, b));
    };
    ...
    if (target.source.kind == MixedFilamentSourceKind::Custom && target.source.origin_auto && valid_auto_pair) {
        bool tombstoned_existing_auto = false;
        for (size_t idx = 0; idx < definitions.size(); ++idx) {
            if (idx == mixed_index) continue;
            MixedFilamentDefinition& candidate = definitions[idx];
            if (candidate.source.kind == MixedFilamentSourceKind::Custom) continue;
            const MixedFilamentPrimaryPairView candidate_pair = candidate.recipe.blend.primary_pair_or();
            if (canonical_pair(candidate_pair.component_a.id, candidate_pair.component_b.id) != target_pair)
                continue;
            candidate.visibility.tombstoned = true;
        ...
        if (tombstoned_existing_auto) {
            definitions.erase(definitions.begin() + mixed_index);
        } else {
            target.recipe.kind = MixedFilamentRecipeKind::WeightedBlend;
            target.recipe.blend.components = {
                {MixedFilamentPhysicalRef{target_pair.first}, 50},
    ...
    std::vector<std::string> physical_colors;
    for (const auto& p_fil : p->m_sidebar_filament_menu->get_physical_filaments()) {
        physical_colors.push_back(p_fil.first);
    }
    ...
    const std::string serialized = mgr.serialize_custom_entries();
    if (ConfigOptionString* opt = pb.project_config.option<ConfigOptionString>("mixed_filament_definitions"))
        opt->value = serialized;
    else
        pb.project_config.set_key_value("mixed_filament_definitions", new ConfigOptionString(serialized));

    pb.update_mixed_filament_id_remap(old_mixed, num_physical, num_physical);

    if (auto* print_tab = wxGetApp().get_tab(Preset::TYPE_PRINT))
        print_tab->update_dirty();
    if (wxGetApp().mainframe)
        wxGetApp().mainframe->on_config_changed(&pb.project_config);
...
void Sidebar::delete_filament(size_t filament_id, int replace_filament_id)
{
    ...
    unsigned int del_phys_1based = (unsigned int)(filament_id + 1);
    std::vector<size_t> dependent_mixed_indices = pb.mixed_filaments.mixed_filaments_using_physical(del_phys_1based);
    int ref_count = (int)dependent_mixed_indices.size();

    // Check if physical filament is in use
    bool phys_in_use = is_filament_in_use(del_phys_1based);

    // Check if any dependent mixed filament is in use
    bool any_dep_in_use = false;
    std::vector<unsigned int> dependent_virtual_ids;
    const auto& mfs = pb.mixed_filaments.mixed_filaments();
    size_t running_idx = 0;
    for (size_t j = 0; j < mfs.size(); ++j) {
        if (mfs[j].deleted || !mfs[j].enabled) continue;
        unsigned int virtual_id = (unsigned int)(num_physical + running_idx + 1); // 1-based
        running_idx++;
        if (std::find(dependent_mixed_indices.begin(), dependent_mixed_indices.end(), j) != dependent_mixed_indices.end()) {
            dependent_virtual_ids.push_back(virtual_id);
            if (is_filament_in_use(virtual_id))
                any_dep_in_use = true;
        }
    }

    bool has_color_or_painting = phys_in_use || any_dep_in_use;
    bool has_dependents = (ref_count > 0);

    int chosen_target_id = replace_filament_id;
    if (replace_filament_id < 0) {
        if (has_color_or_painting || has_dependents) {
            // Build choices for surviving filaments
            std::vector<FilamentChoiceItem> choices;
            std::vector<wxBitmap*> icons = get_extruder_color_icons(true);
            const auto& filament_presets = pb.filament_presets;
            int filaments_cnt = std::min((int)icons.size(), (int)filament_presets.size());
            ...
            for (int i = 0; i < filaments_cnt; i++) {
                if (size_t(i) == filament_id) continue;
                auto preset = pb.filaments.find_preset(filament_presets[i]);
                wxString item_name = preset ? from_u8(preset->label(false)) : wxString::Format(_L("Filament %d"), i + 1);
                choices.push_back({ item_name, *icons[i], size_t(i) });
            }
            ...
            for (size_t j = 0; j < mfs.size(); ++j) {
                if (mfs[j].deleted || !mfs[j].enabled) continue;
                size_t target_vid = num_physical + r_idx;
                r_idx++;

                // Skip mixed filaments that depend on the to-be-deleted physical filament
                if (std::find(dependent_mixed_indices.begin(), dependent_mixed_indices.end(), j) != dependent_mixed_indices.end())
                    continue;
            ...
            if (has_dependents) {
                ...
                message = _L("This filament is currently in use.\nAre you sure you want to delete it?");
            }

            DeleteFilamentDialog dlg(wxGetApp().mainframe, _L("Confirm Delete"), message, has_color_or_painting, choices, 0);
            if (dlg.ShowModal() != wxID_OK) {
                return;
            }

            if (has_color_or_painting) {
                chosen_target_id = dlg.get_target_filament_id();
            }
        }
    }
    ...
    // If a target was selected to receive color/painting information:
    if (chosen_target_id >= 0) {
        transfer_filament(filament_id, size_t(chosen_target_id));
        for (unsigned int dep_vid : dependent_virtual_ids) {
            transfer_filament(size_t(dep_vid - 1), size_t(chosen_target_id));
        }
    }

    if (pb.is_the_only_edited_filament(filament_id) || (filament_id == 0)) {
        wxGetApp().get_tab(Preset::TYPE_FILAMENT)->select_preset(pb.filament_presets[0], false, "", true);
    }
    ...
    wxGetApp().plater()->on_filaments_delete(filament_count, filament_id, -1);
