...
{
    const float line_width = m_perimeter_width * m_filpar[old_tool].ramming_line_width_multiplicator;

    const float xl = m_perimeter_width / 2.f + line_width;
    const float xr = m_wipe_tower_width - m_perimeter_width / 2.f - line_width;
    ...
    if (!m_semm && m_filpar[old_tool].multitool_ramming) {
        const float cross_sect  = layer_height * (line_width - layer_height * (1.f - (float) M_PI / 4.f));
        const float total_len   = cross_sect > 0.f ? m_filpar[old_tool].multitool_ramming_volume / cross_sect : 0.f;
        const float tower_width = xr - xl;
        if (tower_width <= WT_EPSILON)
            return xr;

        const int   full_lines = int(total_len / tower_width);
        const float remainder  = total_len - full_lines * tower_width;
        return (full_lines % 2 == 0) ? (xl + remainder) : (xr - remainder);
    }

    const int ramming_lines = int(m_filpar[old_tool].ramming_speed.size());
    return (ramming_lines % 2 == 1) ? xr : xl;
}
...
void WipeTower2::get_all_wall_skip_points()
{
    ...
    for (size_t layer_id = 0; layer_id < m_plan.size(); ++layer_id) {
        ...
        for (size_t tc_idx = 0; tc_idx < layer.tool_changes.size(); ++tc_idx) {
            ...
            const bool do_ramming = (m_semm && m_enable_filament_ramming) || m_filpar[tc.old_tool].multitool_ramming;
            const float x = (predict_ramming_end_x((int) tc.old_tool, layer.height) < m_wipe_tower_width / 2.f) ?
                                0.f :
                                m_wipe_tower_width;
            const float y = process_depth + (do_ramming ? tc.ramming_depth : 0.f) + m_perimeter_width / 2.f;
