...
public:
    ...
    // Perimeter path modulation V2 keeps an immutable source envelope so a
    // perimeter-only reslice never modulates already modulated geometry.
    ExPolygons                  image_map_unmodulated_raw_slices;
    float                       image_map_external_perimeter_width_mm{0.f};
    bool                        image_map_adaptive_perimeter_island{false};
    SurfaceCollection           image_map_perimeter_fallback_slices;
    bool                        image_map_has_perimeter_fallback_slices{false};
